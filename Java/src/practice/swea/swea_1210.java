		...
		for (int t = 1; t < 11; t++) {
			...
			while (y != 0) {  // 위에 도달할 때까지
				if (-1 < x - 1 && ladder[y][x-1] == 1) {  // 유효 범위이며 왼쪽으로 이동할 수 있다면
					d = 1;
					while(-1 < x - 1 && ladder[y + direction[d][0]][x + direction[d][1]] == 1) {  // 유효 범위이며 왼쪽으로 이동이 다 될 때까지
						y += direction[d][0];
						x += direction[d][1];
					}
				}
					...
					d = 2;
					while(x + 1 < 100 && ladder[y + direction[d][0]][x + direction[d][1]] == 1) {  // 유효 범위이며 오른쪽으로 이동이 다 될 때까지
						y += direction[d][0];
						x += direction[d][1];
					}
