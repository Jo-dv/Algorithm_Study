...
public class boj_17281 {
	...
	public static void main(String[] args) throws IOException {
		...
		order[3] = 0; pick[3] = true;
		perm(1);  // 1번 선수 다음부터 이뤄나가야 하므로
//		perm_bitmask(1, 1 << 3);
	...
	static void perm_bitmask(int target, int mask) {
		if(target == 9) {
			int get_score = play(order);
			answer = answer < get_score ? get_score : answer;
			return;
		...
		for(int i = 0; i < 9; i++) {
			...
			order[i] = player[target];
			perm_bitmask(target + 1, mask | 1 << i);
		}
	}
	...
	static int play(int[] players) {  // 게임 진행 로직
		...
		for (int[] now_inning : inning) {  // 각 이닝에 대한 정보를 가져와서
			...
			while(out < 3) {  // 해당 이닝이 끝나기 전까지 -> out이 3개
				int current_hitter = players[num++];  // 현재 타자
